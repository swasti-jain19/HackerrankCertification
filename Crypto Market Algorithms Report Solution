WITH quarterly_volume AS (
    SELECT 
        ...
        QUARTER(dt) AS quarters  -- Use QUARTER() for extracting quarter
    FROM coins c
    JOIN transactions t ON t.coin_code = c.code
    WHERE YEAR(dt) = 2020  -- Use YEAR() to extract year
    GROUP BY algorithm, QUARTER(dt)  -- Use QUARTER() for grouping by quarter
)
SELECT
       c.algorithm,
